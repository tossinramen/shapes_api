...
async function main() {
  try {
    ...
    if (!process.env.SHAPESINC_SHAPE_API_KEY) {
      throw new Error("SHAPESINC_SHAPE_API_KEY not found in .env");
    }

    // Create the client with the shape API key and the Formless API base URL
    const openai = new OpenAI({
      apiKey: process.env.SHAPESINC_SHAPE_API_KEY,
